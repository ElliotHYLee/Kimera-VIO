...
def make_preint_params_from_config(imu_config):
    # world gravity in z (change if your dataset uses something else)
    params = gtsam.PreintegrationParams.MakeSharedU(0)

    gyro_sigma = imu_config["gyro_sigma"]

    # we "omit" the accelerometer by making it extremely noisy
    huge_acc_sigma = 1e6
    params.setAccelerometerCovariance((huge_acc_sigma ** 2) * np.eye(3))

    # keep real gyro
    params.setGyroscopeCovariance((gyro_sigma ** 2) * np.eye(3))

    # keep small integration noise
...
def main():
    ...
    # we are omitting accel, so just zeros
    acc_bias_vec = np.zeros(3)
    ...
    print(
        ...
        f"  acc_bias(omitted)={acc_bias_vec.tolist()}, gyro_bias={gyro_bias_vec.tolist()}\n"
        f"  gyro_sigma={gyro_sigma}, cam_sigma={cam_sigma}"
    )
    ...
    for i in range(1, len(vio_data)):
        ...
        # we ignore accel from the dataset and only use gyro
        gyro_body = np.array(meas_i["gyro_xyz"])
        has_cam = meas_i["has_cam"]
        uvu = meas_i["uvu"]

        # integrate IMU with zero accel (i.e. accel omitted)
        imu_preintegrator.integrateMeasurement(np.zeros(3), gyro_body, dt)

        if has_cam:
            new_idx = current_state_idx + 1

            # 1) IMU factor (will contain only gyro info effectively)
            ...
            for _ in range(2):
                isam.update(gtsam.NonlinearFactorGraph(), gtsam.Values())
                result = isam.calculateEstimate()
